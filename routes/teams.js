...
router.post("/updateTeam", function (req,res) {
    var data = {name:req.body.name, year:req.body.year, id:req.body.id};
    pg.connect(process.env.DATABASE_URL+"?ssl=true", function (err, client, done) {
        if (err) throw err;
...
router.get("/view",function (req, res) {
    pg.connect(process.env.DATABASE_URL+"?ssl=true", function (err, client, done) {
        ...
        client.query("SELECT * FROM teams order by team_id", function (err, result) {
            ...
            if (err) throw err;
            res.render("pages/teams", {results: result.rows});
        });
    });
});
/*
...
router.get("/view/:id", function (req, res) {
    pg.connect(process.env.DATABASE_URL+"?ssl=true", function (err, client, done) {
        ...
        client.query("select * from teams inner join users on users.teamid = teams.team_id"+
            ...
            if (err) throw err;
            res.render("pages/team", {results: result.rows, teamName : teamName});
        });
    });
});
